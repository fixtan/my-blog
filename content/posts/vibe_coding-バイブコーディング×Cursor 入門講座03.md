...
2024年以降、OpenAI APIの無料枠（$5相当クレジット）は廃止されたため、現在は 課金しない限り API の利用は不可 になっています。

この為、これ以上の動作テストをする事は難しい事が分かりました。

注意点としては、ChatGPT Plusへの課金と、OpenAI APIへの課金は別枠であるという事。
なので、ChatGPTを課金契約していても、OpenAIのAPIが使えるわけではないようです。

この辺りの情報はネット上に結構ありますので、調べると多数出てきます。

以前、無料でAPIを利用した経験があるので、てっきり使えるものとばかり思ってましたが、暫く見てない間に状況が変わったようです。

大きな金額ではないので課金も考えましたが、WEBで公開してしまうと直ぐ上限に達しそうなのと、今後APIを利用する場面がなさそうなので、別の用途で課金する必要が出た場合、この件の続きを記事にしてまとめる予定です。
